...
use crate::common::{
    aggregate::key::{SearchKey, UniqueKey},
    event::Event,
...
impl<T> AggregateId for T where T: Clone + Serialize + for<'a> Deserialize<'a> {}
    ...
    A: Aggregate,
{
    ...
    pub fn handle(mut self, command: A::Command) -> anyhow::Result<Self> {
        let events = A::handle(self.state.as_ref(), command)?;
        let state = events
            .iter()
